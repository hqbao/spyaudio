...
* **Request**: Form-data with `'audio'` file part and `device_id` field.
...
* **Description**: Get a list of all uploaded audio files, sorted by creation date (most recent first).
...
* **Description**: Retrieve the current command for a specific `device_id`. The command resets to `"WAIT"` after reading if it was an active command.

* **Parameters**: `device_id` (query parameter)
...
* **Description**: Set a new command for a target device.

* **Request**: JSON body with `device_id`, `command`, and `message` fields.
